...
public class HttpResponseSettingConfiguration implements HttpResponseSetting {
    protected ResponseHandler handler;
    protected List<MocoEventTrigger> eventTriggers = newArrayList();

    public HttpResponseSetting response(final String content) {
        return this.response(text(checkNotNullOrEmpty(content, "Content should not be null")));
    }

    public HttpResponseSetting response(final Resource resource) {
        return this.response(with(checkNotNull(resource, "Resource should not be null")));
    }

    public HttpResponseSetting response(final ResponseHandler handler) {
    ...
    public HttpResponseSetting response(final MocoProcedure procedure) {
        this.response(with(checkNotNull(procedure, "Procedure should not be null")));
        return this;
    }

    public HttpResponseSetting response(final ResponseHandler... handlers) {
    ...
    public HttpResponseSetting on(final MocoEventTrigger trigger) {
